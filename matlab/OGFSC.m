...
varList = {'nBins', 'minBinSize', 'LR_p', 'plot_option'};
convgThrh = {30, 100, 0.01, 1}; % default values
if ~isempty(varargin)
    L = length(varargin);
    for i = 1:2:L-1
...
plot_option = convgThrh{4}; % plot the outputs of OGFSC, by default 0. 
alpha = 0.5; % list of different threshold, i.e., the upper boundary of confidence interval
...
idx = find(cv2_output>cv2_hat*chi2inv(alpha,df)/df);
OGFSC_idx = idx_output(idx);
cv2_threshold = cv2_hat*chi2inv(alpha,df)/df;
